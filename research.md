...
Example system showing the BBFS criterion, used in AutoMeKin to pindown the location of transition-state guess structures. Blue vectors correspond to the velocity vectors of the specific atom. Solid lines link neighboring atoms and the dashed lines indicate nonneighboring ones. At time step t, the maximum distance between i<sub>1</sub> and its neighbors i<sub>2</sub>, i<sub>3</sub> is smaller than the minimum distance, d, to non-neighbors k;  red dashed line. At time step t', the distance d$\scriptstyle{(}$i<sub>1</sub>,k<sub>2</sub>$\scriptstyle{)}$ becomes smaller than the distance d$\scriptstyle{(}$i<sub>1</sub>,i<sub>3</sub>$\scriptstyle{)}$, indicating the possibility of a TS in the vicinity.

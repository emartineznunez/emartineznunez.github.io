...
Example system showing the BBFS criterion, the one used in AutoMeKin to pindown the location of transition-state guess structures. Blue vectors correspond to the velocity vectors of the specific atom. Solid lines link neighboring atoms and the dashed lines indicate nonneighboring ones. At time step t, the maximum distance between i1 and its neighbors i2, i3 is smaller than the minimum distance (d) to nonneighbors k (red dashed line). At time step t', the distance d(i1,k2) (red solid line) has become smaller than the distance d(i1,i3) (black dotted line), indicating the possibility of a TS in the vicinity
